...
import ru.adnr.converter.storage.service.MinioStorageService;
...
public class ConversionService {
    ...
    private void processRegisteredMessage(FileConversionRequestedEvent event)
        ...
        byte[] pdfContent;
        try (InputStream inputStream = minioStorageService.download(sourceBucket, event.objectKey())) {
        ...
        minioStorageService.upload(pdfBucket, pdfObjectKey, pdfContent, PDF_CONTENT_TYPE);
    ...
    private void validate(FileConversionRequestedEvent event) {
        if (event == null) {
